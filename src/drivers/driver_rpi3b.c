...
struct wpa_driver_rpi3b_data
{
	void *ctx;
	bcm4343_net_device *netdev;
	size_t ssid_len;
	u8 ssid[32];
	int country_set;
};

typedef struct  wpa_driver_rpi3b_data wpa_driver_rpi3b_data;
typedef union wpa_event_data wpa_event_data;
typedef struct wpa_driver_scan_params wpa_driver_scan_params;
...
static int wpa_driver_rpi3b_get_bssid (void *priv, u8 *bssid)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);

	// assert (drv->netdev != 0);
	const u8 *mac = get_mac_address(drv->netdev);

	// // assert (mac != 0);
	// // assert (bssid != 0);
	// mac->CopyTo (bssid);
	os_memcpy(bssid, mac, 6); //Mac address size
	return 0;
}

static int wpa_driver_rpi3b_get_ssid (void *priv, u8 *ssid)
{
	struct wpa_driver_rpi3b_data *drv = (struct wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);
...
static int wpa_driver_rpi3b_set_key (void *priv, wpa_alg alg, const u8 *addr,
				      ...
				      const u8 *key, size_t key_len)
{
	...
	c_string_t *Number = get_new_cstring("");
	c_string_t *Address = get_new_cstring("");

	for (int i = 0; i < ETH_ALEN; i++)
	{
		// Number.Format (, addr[i]);
		cstring_format(Number, "%02X", addr[i]);
		append_to_cstring(Address, Number);
		// Address.Append (Number);
	}

	// // assert (alg == WPA_ALG_TKIP || alg == WPA_ALG_CCMP);

	c_string_t *Key = get_new_cstring(alg == WPA_ALG_TKIP ? "tkip:" : "ccmp:");

	// assert (key_len > 0);
	for (unsigned i = 0; i < key_len; i++)
	{
		cstring_format(Number, "%02X", key[i]);
		append_to_cstring(Key, Number);
	}

	// Key.Append ();
	append_to_cstring(Key, "@");
	...
	for (int i = seq_len-1; i >= 0; i--)
	{
		...
		// Key.Append (Number);
		cstring_format(Number, "%02X", seq[i]);
		append_to_cstring(Key, Number);
	}
	...
	{
		// // assert (key_idx == 0);
		Command = get_new_cstring("txkey");
	}
	else
	{
		// // assert (key_idx <= 3);
		Command = get_new_cstring("");
		cstring_format(Command, "rxkey%u", key_idx);
	}

	if (!wifi_control(drv->netdev, "%s %s %s", (const char *) Command->buffer,
				   (const char *) Address->buffer, (const char *) Key->buffer))
	{
		return -1;
	}
...
static void wpa_driver_rpi3b_event_handler (ether_event_type_t		 type,
					     const ether_event_params_t *params,
					     void			*context)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) context;
	// assert (drv != 0);
...
static void *wpa_driver_rpi3b_init (void *ctx, const char *ifname)
{
	bcm4343_net_device *netdev = get_bcm4343_device();
	if (netdev == 0)
	{
		return 0;
	}

	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) os_zalloc (sizeof *drv);
	if (drv == 0)
	{
		return 0;
	}
	...
	drv->ctx = ctx;
	drv->netdev = (bcm4343_net_device *) netdev;	// netdev can only be of this type
	drv->country_set = 0;

	register_event_handler(drv->netdev, wpa_driver_rpi3b_event_handler, drv);

	return drv;
}

static void wpa_driver_rpi3b_deinit (void *priv)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// // assert (drv != 0);

	register_event_handler(drv->netdev, 0, 0);
...
static void wpa_driver_rpi3b_scan_timeout (void *eloop_ctx, void *timeout_ctx)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) eloop_ctx;
	// assert (drv != 0);

	// assert (drv->netdev != 0);
	// drv->netdev->Control ("escan 0");	// stop scan
	wifi_control(drv->netdev,"escan %d", 0);
	wpa_supplicant_event (timeout_ctx, EVENT_SCAN_RESULTS, 0);
}

static int wpa_driver_rpi3b_scan2 (void *priv, wpa_driver_scan_params *params)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);
	...
	if (!wifi_control(drv->netdev,"escan %u", SCAN_DURATION_SECS+2))
	{
		return -1;
	}

	eloop_cancel_timeout (wpa_driver_rpi3b_scan_timeout, drv, drv->ctx);
	eloop_register_timeout (SCAN_DURATION_SECS, 0, wpa_driver_rpi3b_scan_timeout,
				drv, drv->ctx);
...
typedef struct wpa_scan_results wpa_scan_results;
typedef struct wpa_scan_res wpa_scan_res;

static wpa_scan_results *wpa_driver_rpi3b_get_scan_results2 (void *priv)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);
	...
	// assert (drv->netdev != 0);
	while (receive_scan_results(buf, &len))
	{
...
static int wpa_driver_rpi3b_disassociate (void *priv, const u8 *addr, int reason_code)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);

	// assert (drv->netdev != 0);
	if (!wifi_control(drv->netdev,"disassoc %d", reason_code))
	{
		return -1;
	}
...
typedef struct wpa_driver_associate_params wpa_driver_associate_params;

static int wpa_driver_rpi3b_associate (void *priv, wpa_driver_associate_params *params)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);
	// assert (params != 0);

	c_string_t *BSSID = get_new_cstring("FFFFFFFFFFFF");
	if (params->bssid != 0)
	{
		reinit_cstring(BSSID, "");
		for (unsigned i = 0; i < ETH_ALEN; i++)
		{
			c_string_t *Number = get_new_cstring("");
			cstring_format(Number, "%02X", (unsigned) params->bssid[i]);
			append_to_cstring(BSSID, Number);
			// BSSID.Append (Number);
		}
	}
	...
	c_string_t *SSID = get_new_cstring(ssid);
	replace_in_cstring(SSID, " ", "\\x20");
	// SSID.Replace (" ", "\\x20");
	...
	c_string_t *Auth = get_new_cstring("off");
	if (params->wpa_ie != 0 && params->wpa_ie_len > 0)
	{
		...
		{
			c_string_t *Number = get_new_cstring("");
			cstring_format(Number, "%02X", (unsigned) params->wpa_ie[i]);
			append_to_cstring(Auth,Number);
		}
	}
	...
	// assert (drv->netdev != 0);
	if (!wifi_control(drv->netdev,"join %s %s %u %s", (const char *) SSID->buffer,
				   (const char *) BSSID->buffer, chan, (const char *) Auth->buffer))
	{
		return -1;
	}
...
static int wpa_driver_rpi3b_set_country (void *priv, const char *alpha2)
{
	wpa_driver_rpi3b_data *drv = (wpa_driver_rpi3b_data *) priv;
	// assert (drv != 0);
	...
	// assert (drv->netdev != 0);
	if (!wifi_control(drv->netdev,"country %s", country))
	{
		return -1;
	}
...
const struct wpa_driver_ops wpa_driver_rpi3b_ops =
{
	.name = "rpi3b",
	.desc = "RPI3B WLAN driver",
	.get_bssid = wpa_driver_rpi3b_get_bssid,
	.get_ssid = wpa_driver_rpi3b_get_ssid,
	.set_key = wpa_driver_rpi3b_set_key,
	.init = wpa_driver_rpi3b_init,
	.deinit = wpa_driver_rpi3b_deinit,
	.disassociate = wpa_driver_rpi3b_disassociate,
	.associate = wpa_driver_rpi3b_associate,
	.get_scan_results2 = wpa_driver_rpi3b_get_scan_results2,
	.set_country = wpa_driver_rpi3b_set_country,
	.scan2 = wpa_driver_rpi3b_scan2,
};
