...
struct l2_packet_data * l2_packet_init (const char *ifname, const u8 *own_addr, unsigned short protocol,
				 ...
				 void *rx_callback_ctx, int l2_hdr)
{
	...
	struct l2_packet_data *l2 = (struct l2_packet_data *) os_zalloc (sizeof *l2);
	if (l2 == 0)
	{
		return 0;
	}
	...
	const u8 *mac = get_mac_address(get_bcm4343_device());

	// assert (mac != 0);
	// mac->CopyTo (l2->own_addr);
	os_memcpy(l2->own_addr, mac, MAC_ADDRESS_SIZE); //

	// l2->link = CNetSubSystem::Get ()->GetLinkLayer ();
	// assert (l2->link != 0);
	if (!enable_receive_raw_l2(protocol))
	{
		os_free (l2);
...
int l2_packet_send (struct l2_packet_data *l2, const u8 *dst_addr, u16 proto, const u8 *buf, size_t len)
{
	...
	// assert (len > 0);
	if (len + sizeof (struct TEthernetHeader) > FRAME_BUFFER_SIZE)
	{
		return -1;
	}

	// assert (buf != 0);
	os_memcpy (Buffer + sizeof (struct TEthernetHeader), buf, len);

	// assert (l2->link != 0);
	send_raw_l2_packet(Buffer, len + sizeof (struct TEthernetHeader));
...
static void l2_packet_receive (int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	u8* Sender;
	while (receive_raw_l2_packet(Buffer, &nResultLength, Sender))
	{
		u8 src_addr[MAC_ADDRESS_SIZE];
		os_memcpy(src_addr, Sender, MAC_ADDRESS_SIZE); //
